...
echo "DATE,USAGE" > upload-usage.txt
...
echo -n `date | awk '{print $4}'`"," >> upload-usage.txt
echo "$kbps" >> upload-usage.txt

if [ "$(cat upload-usage.txt | wc -l)" == "62" ]
then
sed -i -e '2d' upload-usage.txt
fi
